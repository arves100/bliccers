...
u32 BrickNand(void)
{
	u32 nandsize = getMMCDevice(0)->total_size * NAND_SECTOR_SIZE;
	u32 sectors = nandsize / NAND_SECTOR_SIZE;

	for (u32 i = 0; i < sectors; i += SECTORS_PER_READ) {
            u32 read_sectors = min(SECTORS_PER_READ, (sectors - i));
			u8 buffer[read_sectors];
			
			// Q U A L I T Y  C O D E
			for (u32 k = 0; k < read_sectors; k++)
				buffer[k] = 0x00;
